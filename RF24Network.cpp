...
void RF24Network::update(void)
{
  ...
  while ( radio.available(&pipe_num) )
  {
    ...
    while (!done)
    {
      ...
      IF_SERIAL_DEBUG(printf("%lu: MAC Received on %u %s\n\r",millis(),pipe_num,header.toString()));
      IF_SERIAL_DEBUG(const uint16_t* i = reinterpret_cast<const uint16_t*>(frame_buffer + sizeof(RF24NetworkHeader));printf_P("%lu: NET message %04x\n\r",millis(),*i));
...
const char* RF24NetworkHeader::toString(void) const
{
  ...
  IF_SERIAL_DEBUG(snprintf_P(buffer,sizeof(buffer),PSTR("id %04x from 0%o to 0%o type %c"),id,from_node,to_node,type));
  return buffer;
}
